...
public class Equation3View extends FormulaView {
    ...
    private LinearLayout llyEquationRoot, llyEquation1, llyEquation2, llyEquation3;

    public Equation3View(Context context, int level) {
        super(context, level);
        initView();
    }

    public Equation3View(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public Equation3View(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView();
    }

    private void initView(){
        Log.d(TAG, "--->添加三项方程式公式, 级别：" + level);
        ...
        llyEquation3 = (LinearLayout) findViewById(R.id.llyEquation3);
        ...
        setCanClickView(llyEquation3, false, false);
    }
    ...
    public void toLatexString(ConvertResult convertResult) {
        ...
        //添加分母
        convertResult.message += LatexConstant.Brace_Left;
        toLatexString(llyEquation3, convertResult);
        if (!convertResult.isSuccess) {
            return;
        }
        convertResult.message += LatexConstant.Brace_Right;
